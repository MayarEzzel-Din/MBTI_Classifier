...
MODELS_DIR = ""
DATA_DIR = ""
TEST_TWEETS_PATH = os.path.join(DATA_DIR, "TestingData.csv")
...
x_test = []
with open(TEST_TWEETS_PATH, "r", encoding="ISO-8859-1") as f:
    reader = csv.reader(f)
    for row in f:
        x_test.append(row)
...
for k in range(len(DIMENSIONS)):
    model = load_model(os.path.join(MODELS_DIR, "rnn_model_{}.h5".format(DIMENSIONS[k])))
    tokenizer = None
    with open(os.path.join(MODELS_DIR, "rnn_tokenizer_{}.pkl".format(DIMENSIONS[k])), "rb") as f:
