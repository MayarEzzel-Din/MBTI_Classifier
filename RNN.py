...
for k in range(len(DIMENSIONS)):
    ...
    model = Sequential()
    model.add(Embedding(TOP_WORDS, EMBEDDING_VECTOR_LENGTH, input_length=MAX_POST_LENGTH, weights=[embedding_matrix],
                        mask_zero=True, trainable=True, ))
    model.add(LSTM(EMBEDDING_VECTOR_LENGTH, dropout=DROPOUT, recurrent_dropout=DROPOUT, activation="relu",
    ...
    if CROSS_VALIDATION:
        ...
        for train_indices, test_indices in k_fold:
            ...
            with open(os.path.join(DATA_DIR, "rnn_cross_validation_{}.txt".format(DIMENSIONS[k])), "w",
                      encoding="utf-8") as f:
                f.write(
    ...
    model.fit(preprocess(df["x"].values), df["y"].values, epochs=10, batch_size=MODEL_BATCH_SIZE, )
    predictions = model.predict_classes(preprocess(x_test))
    confusion = confusion_matrix(y_test, predictions)
    score = accuracy_score(y_test, predictions)
    with open(os.path.join(MODELS_DIR, "rnn_accuracy_{}.txt".format(DIMENSIONS[k])), "w", encoding="utf-8") as f:
    ...
    if WORD_CLOUD:
        ...
        with open(os.path.join(DATA_DIR, "extreme_examples_{}.txt".format(DIMENSIONS[k][0])), "w",
                  encoding="utf-8") as f:
            for prob, i in min_prob_indices:
                f.write("\n")
        with open(os.path.join(DATA_DIR, "extreme_examples_{}.txt".format(DIMENSIONS[k][1])), "w",
                  encoding="utf-8") as f:
    ...
    with open(os.path.join(MODELS_DIR, "rnn_tokenizer_{}.pkl".format(DIMENSIONS[k])), "wb") as f:
